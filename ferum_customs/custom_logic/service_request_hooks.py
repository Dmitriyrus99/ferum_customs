...
def _notify_project_manager(doc: ServiceRequest) -> None:
	...
	try:
		...
		if customer_email:
			...
			frappe.logger(__name__).warning(f"Service Request '{doc.name}' has no customer_email field set")

		if not recipients:
			frappe.logger(__name__).warning(
				_("Получатели с ролью '{0}' не найдены для уведомления.").format(ROLE_PROEKTNYJ_MENEDZHER)
			)
			return
