...
def iter_source_files(
    ...
    if not include_list_path.is_file():
        console.print(
            f"[red]Файл со списком для обработки не найден: {include_list_path}[/red]"
        )
        return
    ...
    target_files = {
        pathlib.Path(line.strip()) for line in target_files_str if line.strip()
    }
    ...
    for p in target_files:
        ...
        if full_path.is_file():
            ...
            console.print(
                f"[yellow]⚠️  Файл из списка не найден, пропускаю: {p}[/yellow]"
            )
