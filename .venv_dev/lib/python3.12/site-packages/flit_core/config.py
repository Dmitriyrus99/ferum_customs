...
def _prep_metadata(md_sect, path):
    ...
    for e, reqs in reqs_extra.items():
        if not all(isinstance(a, str) for a in reqs):
            raise ConfigError(f"Expected a string list for requires-extra group {e}")
        if not name_is_valid(e):
            raise ConfigError(f"requires-extra group name {e!r} is not valid")
        enorm = normalise_core_metadata_name(e)
        extra_names_by_normed.setdefault(enorm, set()).add(e)
        res.reqs_by_extra[enorm] = reqs

    clashing_extra_names = [g for g in extra_names_by_normed.values() if len(g) > 1]
    if clashing_extra_names:
        fmted = ["/".join(sorted(g)) for g in clashing_extra_names]
        raise ConfigError(f"requires-extra group names clash: {'; '.join(fmted)}")
...
def _license_files_from_globs(project_dir: Path, globs, warn_no_files=True):
    ...
    for pattern in globs:
        ...
        try:
            ...
            raise ConfigError(
                "Invalid glob pattern for [project.license-files]: '{}'. {}".format(
                    pattern, ex.args[0]
                )
            )
        ...
        if not files and warn_no_files:
            raise ConfigError(
                "No files found for [project.license-files]: '{}' pattern".format(
                    pattern
                )
            )
...
def read_pep621_metadata(proj, path) -> LoadedConfig:
    ...
    if "license" in proj:
        ...
        if isinstance(proj["license"], str):
            ...
            md_dict["license_expression"] = normalise_compound_license_expr(
                licence_expr
            )
        else:
            ...
            if "file" in license_tbl:
                ...
                if not license_p.is_file():
                    raise ConfigError(
                        f"License file {license_tbl['file']} does not exist"
                    )
